...
def check_winner(player):
    ...
    To check whether game is won by user 1 or user 2.
...
def motion(event):
    ...
    for square,square_limits in coord_dict.items():
        if(x > square_limits[0][0] and x < square_limits[0][1]) and (y > square_limits[1][0] and y < square_limits[1][1]):
            if square in squares_left:
                #User X or 0 placed
                draw(square_limits,turn_text)
                squares_left.remove(square)
                game_moves_dict[turn_text].append(square)
                break
            else:
    ...
    #Swap to next player
    if turn == True:
        turn,turn_text = False,"0"
    else:
        turn,turn_text = True,"X"
...
def main():
    ...
    global master,canvas,coord_dict,turn,turn_text,squares_left,game_moves_dict
